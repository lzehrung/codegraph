...
Check it with `codegraph server status --root /path/to/repo --json`, and stop it explicitly with `codegraph server stop --root /path/to/repo`. The wrapper verifies its process with a per-user credential outside the project; an unreachable or identity-mismatched server requires verification and keeps its registry.
