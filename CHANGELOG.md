...
- Added `codegraph server start|status|stop` to manage one project-local MCP HTTP server. The loopback-only default serializes lifecycle changes, records only process metadata after a root, process, and startup-identity health check, distinguishes confirmed stale state from an unreachable server, and requires an explicit restart after a package update.
