...
For absence claims, require `referenceCoverage.state: complete`; `strategy_unavailable` means an applicable scan could not prove all candidate sites. Coverage covers indexed candidates, not unknown dynamic imports. CLI and library import declaration hits label source and alias/default tokens in `via.importBinding`; MCP `refs` flattens this role to top-level `importBinding`.
