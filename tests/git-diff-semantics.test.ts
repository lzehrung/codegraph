...
describe("git diff semantics", () => {
  ...
  it("surfaces invalid git revisions instead of returning empty results", async () => {
    ...
    try {
      ...
      await expect(listChangedFiles(root, { base: "definitely-not-a-ref", head: "HEAD" })).rejects.toThrow(/definitely-not-a-ref/);
      await expect(getUnifiedDiff(root, { base: "definitely-not-a-ref", head: "HEAD" })).rejects.toThrow(/definitely-not-a-ref/);
    } finally {
      await fs.rm(root, { recursive: true, force: true });
    }
  });
