...
- PR review: run `codegraph impact --base origin/main --head HEAD --pretty` for a ranked map, `codegraph review --base origin/main --head HEAD --summary` for a compact reviewer handoff with candidate tests grouped by confidence, or redirect plain `review` output when a downstream tool needs the full JSON bundle.
