...
function toKey(e: any) {
  const toStr = (t: any) => (t.type === 'file' ? t.path : t.name);
  return `${e.from}|${toStr(e.to)}|${e.raw}`;
}
...
describe('Robust fast graph (strings/templates, dynamic import, CJS)', () => {
  it('ignores require inside string literal', async () => {
    ...
    const g = await (await import('../src/graphs.js')).collectGraph(root, [file.replace(/\\/g, '/')], { fast: true });
  ...
  it('detects dynamic import edges', async () => {
    ...
    const files = [main, a].map(f => f.replace(/\\/g, '/'));
    const g = await collectGraph(root, files);
    const keys = new Set(g.edges.map(toKey));
