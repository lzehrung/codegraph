...
Use `refactor rename` for conservative semantic renames when `@lzehrung/codegraph-refactor` is installed with the CLI; it rejects obvious target-name collisions in the declaring file. Use `refactor move` for TypeScript and JavaScript top-level declarations when named ES importers and imports used by the moved declaration should move with the declaration. For rename and move, pass either `--symbol <handle>` from `list-symbols` or `--at <file>:<line>:<column>` when cursor location is easier. Use `refactor extract` for inclusive contiguous TypeScript and JavaScript statement line ranges inside one function body; simple TypeScript parameter annotations are preserved in generated helpers. These commands return deterministic edits by default and reject unsupported shapes instead of guessing; inspect the edits, then run the repo's typecheck or tests after applying. `--text` prints status, reason, and warnings for unsupported or error results.
