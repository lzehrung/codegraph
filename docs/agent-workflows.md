...
- For streaming review packs, keep the default `streamSummary: "full"` when the final pack needs suggestions, export summaries, re-export chains, graph edges, cycles, clusters, and surface area. Use `streamSummary: "light"` when the agent only needs progressive chunks plus final changed/impacted counts and details.
...
- Batch impact wrappers return `format: "full" | "compact"` so downstream prompts can branch on payload shape directly; streaming `complete.report` uses `format: "stream-summary"`.
