...
When the native addon is available, all listed source languages use the same native Tree-sitter runtime and query model. The optional `@lzehrung/codegraph-js-fallback` package is only needed for forced JS fallback mode or when the native addon itself is unavailable; native-only installs do not need it for normal JS, TS, TSX, or Kotlin import extraction, symbol indexing, chunking, or AST grep. When JS-family query recovery degrades in `auto` mode, Codegraph reports that once in diagnostics and uses the native-owned final recovery path where supported.
...
* Native-only installs do not need `@lzehrung/codegraph-js-fallback` for normal JS, TS, TSX, or Kotlin import extraction, symbol indexing, chunking, or AST grep; native-owned recovery is used as the final degraded path when query recovery is unavailable
