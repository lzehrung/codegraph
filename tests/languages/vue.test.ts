...
it("classifies a type-only import inside a Vue script block", async () => {
  const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "cg-vue-type-only-"));
  const sourceFile = path.join(root, "Widget.vue");
