...
Use `--cache disk` for reuse across CLI processes, `--cache memory` for reuse within one process, and `--cache off` for a deliberate cold run. Disk cache requires Node.js 22.16 or newer; older 22.x builds that load `node:sqlite` without the statement APIs Codegraph needs disable disk cache after one warning. `--cache-verify` validates manifest entries before reuse and `--cache-strict` adds content hashing plus full rediscovery; both trade speed for certainty when cache or manifest state is in use. With `--cache off`, there is no reusable state to validate, so `--cache-strict` has no effect. `codegraph init` is optional lifecycle metadata plus cache warmup; query commands do not require `.codegraph/manifest.json`.
...
Successful `refs --json` results include `referenceCoverage`. `complete` means that every statically linked candidate known to the current index was checked and the search was not capped. `partial` includes one or more reasons: `parser_degraded`, `unresolved_import`, `strategy_unavailable`, or `truncated`. `strategy_unavailable` means that an applicable reference strategy could not prove all candidate sites, such as a member access with an unknown receiver type. This scope does not certify dynamic imports or bare imports that the index cannot associate with the target. Human-readable output prints a note only for partial coverage.
...
- For `duplicates`, `--include-glob` and `--ignore-glob` are relative to each active scan root. They filter duplicate candidates without changing the shared project-index cache.
