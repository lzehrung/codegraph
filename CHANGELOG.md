...
- Relative imports whose extensionless TypeScript stem ends in a dotted suffix, such as `./statement-config.model`, now resolve to `.ts` and `.tsx` files. Cross-file navigation and reference searches no longer omit consumers reached through these imports.
