...
- C and C++ quoted includes resolve against the including file's directory, matching the language rule. `#include "lib.h"` and `#include "inc/lib.h"` previously stayed external unless written as `./lib.h`, so dependency edges, go-to-definition, and find references all failed for ordinary C/C++ layouts. Reduced mode now preserves the include as an import binding instead of only a file edge, so navigation and references still receive it when native capture is unavailable. Angle includes resolve only an exact header spelling under configured include roots, without extension or index probing, and macro includes remain external. C is also routed through the language import resolver for graph edges, which previously listed only C++.
