...
function stripIndentedMarkdownCode(source: string): string {
  const quoteListColumns: number[][] = [[]];
  let blankLineCount = 0;
  return source.replace(/^[^\r\n]*/gm, (line) => {
    if (!line.trim()) {
      blankLineCount += 1;
      if (blankLineCount >= 2) {
        quoteListColumns.length = 1;
        quoteListColumns[0] = [];
      }
      return line;
    }
    ...
    let column = 0;
    let quoteDepth = 0;
    let listContentColumns = quoteListColumns[0]!;
    let contentColumn = 0;
    while (true) {
      ...
      if (index === line.length) {
        quoteListColumns.length = quoteDepth + 1;
        return line;
      }
      while (listContentColumns.length && column < listContentColumns[listContentColumns.length - 1]!) {
        listContentColumns.pop();
      }
      contentColumn = listContentColumns[listContentColumns.length - 1] ?? 0;
      if (column >= contentColumn + 4 || line[index] !== ">") break;
      index += 1;
      if (line[index] === " " || line[index] === "\t") index += 1;
      column = 0;
      quoteDepth += 1;
      // Each quote has its own list indentation; closing it discards that state.
      listContentColumns = quoteListColumns[quoteDepth] ??= [];
    }
    quoteListColumns.length = quoteDepth + 1;
    if (column >= contentColumn + 4) return maskMatch(line);
    const content = line.slice(index);
    // A thematic break is not a list marker, even when it uses spaced hyphens.
    if (/^(?:(?:\*[ \t]*){3,}|(?:-[ \t]*){3,}|(?:_[ \t]*){3,})$/.test(content)) return line;
