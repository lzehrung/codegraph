...
import type { CliBaseContext, CliProjectContext } from "./invocation-context.js";
...
export const CLI_COMMAND_TABLE: Readonly<Record<string, CliCommandEntry>> = {
  ...
  "refactor-plan": {
    stage: "project",
    run: async (ctx) => {
      const { handleRefactorPlanCommand } = await import("./refactor-plan.js");
  ...
  "rename-preview": {
    stage: "project",
    run: async (ctx) => {
      const { handleRenamePreviewCommand } = await import("./rename-preview.js");
  ...
  callers: {
    stage: "project",
    run: async (ctx) => {
      const { handleCallHierarchyCommand } = await import("./call-hierarchy.js");
  ...
  supertypes: {
    stage: "project",
    run: async (ctx) => {
      const { handleTypeHierarchyCommand } = await import("./type-hierarchy.js");
  ...
  "graph-delta": {
    ...
    run: async (ctx) => {
      ...
      const { handleGraphDeltaCommand } = await import("./graph-delta.js");
  ...
  deps: {
    ...
    run: async (ctx) => {
      ...
      const { handleGraphQueryCommand } = await import("./graph-queries.js");
