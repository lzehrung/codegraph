...
- Shared callable ranges restore default-argument and variadic call edges in TypeScript/TSX, C#, Java, Kotlin, and Swift. Signature extraction preserves generic parameter types, receiver binding, Ruby block capture, and Rust trait declarations. Unknown spreads remain unknown rather than becoming one argument.
- Navigation and references include same-directory package/namespace peers, C# partial types and members, Swift extension members, and Zig imported functions. Go/Java/Kotlin package identity ignores misleading `package` text in comments and strings. Proven-equivalent C# partial declarations share one lookup target for third-file callers without merging unrelated types or nested enclosing generic arities. Swift `private` and `fileprivate` extension members, including members of a `private` or `fileprivate` extension, stay file-local in shared-owner navigation and detailed graph edges; same-file private members and ordinary cross-file members remain visible. Lookup keeps unrelated declarations separate and reports partial coverage when compilation-unit membership cannot be established beyond the supported source boundary. Zig module exports now require `pub`.
- Swift constrained extensions no longer add members to nominal types with unproven `where` requirements. Navigation and detailed call edges keep calls within a constrained extension, calls to peers with the same constraint syntax despite different spacing or comments, and calls from that extension to visible nominal members; cross-file hidden members remain hidden.
- C# partial members declared in one part resolve from bare calls in another part, and `findReferences` includes those calls. A static caller reaches only static members, and a same-named local function still wins. `namespace @P` and `namespace P` share one namespace for navigation, references, and partial owners. Members of C# partial types and Swift types or extensions report `partial` reference coverage with `strategy_unavailable` when another owner part, or a C# file that can qualify the owner, can sit outside the proven unit directory. A C# type declared beside block-scoped namespaces is a global-namespace type visible to every namespace.
- C# extension-method signatures recognize the leading `this` parameter as the receiver. Calls through the declaring static class, including alias- and namespace-qualified owners, and bare calls inside it count that receiver as an argument in call edges, navigation, and impact hints. C# 11 `file` types are no longer visible to other files in the same namespace, and same-named `file partial` owners in different files no longer share members.
- C# same-file navigation and constructor edges respect namespace boundaries and retain explicit namespace-qualified targets. References include uses in reopened namespace blocks without including unrelated namespaces or local variables.
- C# `using X = Namespace; X::Target` resolves through the namespace alias for go-to-definition, references, and constructor/type graph edges, including verbatim spellings such as `using @X = P;` with `X::Target` and the same alias declared again in another namespace block. Ordinary dotted names and `global::` reach the declared type in the same compilation directory even when the use file also declares unrelated namespaces. Unknown aliases do not select same-spelled types. Qualified reference coverage stays partial when other directories can contain uses.
- Qualified C# generic type names retain dotted, `global::`, and namespace-alias paths through go-to-definition, references, and constructor edges; same-named types in other namespaces stay separate.
...
- PHP dependency extraction, symbol lists, and compact and detailed graphs preserve `class`, `function`, and `const` import roles when targets or aliases share a spelling. Each import handle selects its own definition and references; an untyped PHP named import uses the class role. Detailed graph `uses` edges select the occurrence's role, fold class/function alias case, and keep constant aliases case-sensitive.
...
- C++ call arity is checked even when only one callable exists. Overloaded `using` aliases resolve through matching calls and references, including C++ `.h` headers, while same-signature functions from different namespaces remain distinct. A `using` alias inside an inline namespace is also visible through its enclosing namespace.
- C++ `using` declarations resolve functions in directly included headers instead of selecting the alias declaration. Navigation, references, and call edges preserve overload arity and local shadowing with cache off and with cold and warm disk caches.
- C++ parameter packs accept zero trailing arguments without dropping required fixed parameters. Fixed and variadic signatures remain separate, including callback parameter types. Qualified C++ bases retain their namespace paths in inheritance and inherited-call edges instead of selecting same-named decoys.
- C++ `.h` include expansion preserves C++ import identities instead of applying C tag namespaces. Qualified C++ import handles and namespace-specific C handles resolve through the public ID queries.
- C++ parameter signatures apply array-to-pointer and function-to-pointer adjustments and discard proven top-level `const`/`volatile`. Equivalent declarations share navigation, references, and graph nodes, including callbacks spelled with `(void)` or `()`. Pointee qualifiers, references, inner array extents, and opaque named-type qualifiers remain distinct.
- C exports keep tags separate from same-spelled typedefs and other ordinary names. Function and tag redeclarations still share one export identity. Pointer, array, and function-pointer typedef exports select the declared name rather than a same-spelled tag.
- C navigation and references preserve the tag namespace through header includes. `struct`, `union`, and `enum` uses reach their tag declarations, while bare typedef names reach their separate declarations, including same-spelled enum tags and typedefs. Ordinary local names do not hide tags.
- C file-scope forward declarations reuse included tags, while block-scope forward declarations retain their own scope. Completed tags take precedence over incomplete declarations in exports and local lookup, retaining earlier forward references. Compact and detailed symbol graphs keep separate tag and ordinary-name include aliases, including a function that shares a tag's name.
...
- Kotlin and Swift member calls use accepted callable ranges, including default parameters and trailing closures. Only one arity-compatible overload can produce a call edge; overlapping ranges remain unresolved.
