...
For a reusable local HTTP server, run `codegraph server start --root . --warmup`, inspect it with `codegraph server status --root . --json`, and shut it down with `codegraph server stop --root .`. The lifecycle wrapper writes `.codegraph/server.json` only after `/health` identifies the same root, defaults to `127.0.0.1:7331`, removes stale registry metadata safely, and requires an explicit restart after a package update.
