...
- Receiver-aware method references and call compatibility are JS/TS-only today. They are emitted only for verified receivers such as `new Service().run()` and `const service = new Service(); service.run()`.
