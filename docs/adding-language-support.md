...
- Current subsystem tables include `src/util/trivia-tables.ts`, `src/util/member-access-tables.ts`, `src/util/dynamic-import-tables.ts`, `src/indexer/scope-nodes.ts`, `src/indexer/declaration-visibility.ts`, `src/indexer/imports/import-binding-tables.ts`, `src/languages/callable-arity.ts`, and `src/document-links/html-forms.ts`. Add a reasoned omission row when the subsystem requires complete language coverage.
