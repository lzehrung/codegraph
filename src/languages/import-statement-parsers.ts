...
const RUST_IDENTIFIER_PREFIX_PATTERN = new RegExp(String.raw`^${XID_IDENTIFIER_SOURCE}`, "u");
...
function takeRustIdentifierEnd(source: string, index: number): number | null {
  const match = source.slice(index).match(RUST_IDENTIFIER_PREFIX_PATTERN);
  if (!match) return null;
  return index + match[0].length;
}

function takeRustMacroPathEnd(source: string, index: number): number | null {
  let cursor = takeRustIdentifierEnd(source, index);
  if (cursor === null) return null;
  while (source.startsWith("::", cursor)) {
    const next = takeRustIdentifierEnd(source, cursor + 2);
...
const RUST_DELIMITER_PAIRS: Record<string, string> = { "(": ")", "{": "}", "[": "]" };

export function skipRustBalancedDelimiters(source: string, index: number, closer: string): number {
  const stack = [closer];
  let cursor = index;
...
export function isRustItemStartBoundary(source: string, index: number): boolean {
  if (index <= 0) return true;
  return !/[_\p{XID_Continue}]/u.test(source[index - 1]!);
}

export function skipRustOuterAttribute(source: string, index: number): number | null {
  return skipRustHashAttribute(source, index, false);
}

export function skipRustAttribute(source: string, index: number): number | null {
  return skipRustHashAttribute(source, index, true);
}

function skipRustHashAttribute(source: string, index: number, allowInner: boolean): number | null {
  if (source[index] !== "#") return null;
  let cursor = index + 1;
  while (cursor < source.length && /\s/.test(source[cursor]!)) cursor += 1;
  if (source[cursor] === "!") {
    if (!allowInner) return null;
    cursor += 1;
    while (cursor < source.length && /\s/.test(source[cursor]!)) cursor += 1;
  }
...
export function skipRustMacroTokenTree(source: string, index: number): { end: number } | null {
  const pathEnd = takeRustMacroPathEnd(source, index);
  ...
  cursor += 1;
  while (cursor < source.length && /\s/.test(source[cursor]!)) cursor += 1;
  const nameEnd = takeRustIdentifierEnd(source, cursor);
  if (nameEnd !== null) {
    let afterName = nameEnd;
    while (afterName < source.length && /\s/.test(source[afterName]!)) afterName += 1;
    const openAfterName = source[afterName];
    if (openAfterName === "(" || openAfterName === "{" || openAfterName === "[") {
      cursor = afterName;
    }
  }
  const open = source[cursor];
  if (open === undefined) return null;
  const closer = RUST_DELIMITER_PAIRS[open];
  if (!closer) return null;
  return { end: skipRustBalancedDelimiters(source, cursor + 1, closer) };
}

function rustCommentStartEndingAt(source: string, end: number): number | null {
  if (end >= 2 && source[end - 2] === "*" && source[end - 1] === "/") {
    for (let start = end - 2; start >= 0; start -= 1) {
      if (source[start] !== "/" || source[start + 1] !== "*") continue;
      const skipped = skipRustCommentOrLiteral(source, start);
      if (skipped?.kind === "comment" && skipped.end === end) return start;
    }
    return null;
  }
  ...
  let scan = lineStart;
  while (scan < end) {
    const skipped = skipRustCommentOrLiteral(source, scan);
    if (!skipped) {
      scan += 1;
      continue;
    }
    if (skipped.kind === "comment" && source[scan] === "/" && source[scan + 1] === "/" && skipped.end === end) {
      return scan;
    }
    if (skipped.end > scan) {
      scan = skipped.end;
...
export function skipRustCommentsAndWhitespaceBackward(source: string, index: number): number {
  let cursor = index;
  while (cursor > 0) {
    ...
    const commentStart = rustCommentStartEndingAt(source, cursor);
    if (commentStart === null || commentStart >= cursor) break;
    cursor = commentStart;
  }
  return cursor;
}
...
function takeLeadingRustOuterAttributes(text: string): { attributes: string; rest: string } {
  let index = 0;
  ...
  return { attributes: text.slice(0, index), rest: text.slice(index).trim() };
}
...
export function parseRustImportStatements(stmtText: string): ParsedRustImportStatement[] {
  ...
  const { attributes, rest: statement } = takeLeadingRustOuterAttributes(trimmed);
  const pathAttribute = rustPathAttributeFromText(attributes);
