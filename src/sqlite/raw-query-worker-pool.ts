...
/** Resolves the compiled worker entry the same way `query-index-worker.js` is resolved:
...
export function resolveRawSqlQueryWorkerPath(): string {
  ...
  const sibling = path.resolve(selfDirectory, "raw-query-worker.js");
  if (fs.existsSync(sibling)) return sibling;
  const packageRoot = findPackageRoot(selfDirectory);
  const compiled = path.join(packageRoot, "dist", "sqlite", "raw-query-worker.js");
  if (fs.existsSync(compiled)) return compiled;
  const bundled = path.join(packageRoot, "dist", "bin", "raw-query-worker.js");
