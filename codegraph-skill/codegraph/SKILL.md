...
For absence claims, inspect `referenceCoverage`: it covers indexed candidates, not unknown dynamic imports; partial results may report `strategy_unavailable` when an applicable reference collection strategy could not run or `name_equivalence_unavailable` when language-specific name equivalence could not be verified. CLI and library import declaration hits label source and alias/default tokens in `via.importBinding`; MCP `refs` flattens this role to top-level `importBinding`.
