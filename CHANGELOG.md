...
- Swift constrained extensions no longer add members to nominal types with unproven `where` requirements. Navigation and detailed call edges keep calls within a constrained extension, calls to identically constrained peers, and calls from that extension to visible nominal members; cross-file hidden members remain hidden.
