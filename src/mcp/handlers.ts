...
type McpHandlerFamily<Keys extends keyof CodegraphMcpHandlers> = Pick<CodegraphMcpHandlers, Keys>;
...
export async function createWarmedCodegraphMcpResources(
...
function createCodegraphMcpHandlersForSession(
  ...
  const searchHandlers: McpHandlerFamily<
    | "search"
    | "workspace_symbols"
    | "rename_preview"
    | "refactor_plan"
  ...
  const navigationHandlers: McpHandlerFamily<
    "explore" | "orient" | "packet_get" | "get_file" | "get_symbol" | "goto" | "refs" | "deps" | "rdeps" | "path"
  > = {
  ...
  const reviewHandlers: McpHandlerFamily<"impact" | "review"> = {
  ...
  const artifactHandlers: McpHandlerFamily<"query_sqlite" | "refresh_index" | "artifact_build" | "dispose"> = {
