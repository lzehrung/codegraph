...
describe("inspectDistForTests", () => {
  ...
  test("should require a build when sources are newer than dist artifacts", async () => {
    ...
    try {
      ...
      await fsp.mkdir(path.join(root, "dist", "agent", "query-index"), { recursive: true });
      await setFileMtime(path.join(root, "dist", "agent", "query-index", "queryIndexWorker.js"), distTime, "export {};\n");
