...
| Native-only JS-family runtime coverage | `tests/detailed-symbol-native-only.test.ts`, `tests/chunkFile.behavior.test.ts`, `tests/native-query-scope.test.ts` | JS, TS, and TSX indexing, AST grep, chunking, and module-specifier recovery stay off the optional JS fallback package when the native addon is loaded, while still reporting degraded query recovery in diagnostics. | Internal regression test | 2026-04-25 |
