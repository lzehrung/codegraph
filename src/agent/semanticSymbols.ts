...
export function requireSemanticSymbol(snapshot: AgentProjectSnapshot, input: string): ResolvedSemanticSymbol {
  ...
  if (parseAgentSymbolHandle(input)) {
    throw new Error("Symbol handle is stale or missing. Run workspace_symbols to resolve it again.");
  }
  ...
  throw new Error(
    `Symbol target "${input}" was not found. Run search or workspace_symbols to locate it.`,
  );
}
