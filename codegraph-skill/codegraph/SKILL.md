...
- Index discovery diagnosis: `--report` timing JSON can include `sourceDiscoveryMs`, `metadataDiscoveryMs`, `gitListMs`, `filesystemScanMs`, and `cacheProbeMs` when those steps run. Named `steps` can also include discovery and post-parse persistence durations (`cache-probe`, `persist-cache`, `workspace-manifests`, `index-manifest`, `finalize`, `snapshot-write`); incremental updates include `snapshot-write` when the project snapshot is rewritten.
