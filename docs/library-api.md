...
Use `exploreCodegraph()` when the caller needs one bounded response over search, packets, paths, reverse dependencies, and candidate tests. It makes one default hybrid retrieval pass across symbol names, paths, indexed text chunks, and graph evidence, then derives every other section from its top anchors. It does not decompose a compound natural-language question, plan more searches, infer unstated relations, or prove runtime behavior. `analysis.mode: "semantic"` reports parser-backend quality, not semantic or agentic query reasoning. For compound questions, make separate `searchCodegraph()` calls for concrete terms, then use `explainCodegraphTarget()` or packets for the strongest handles. The response has `schemaVersion: 1`, the original query, `analysis`, summary bullets, anchors, packets, paths, blast radius with per-entry omitted lower bounds, candidate tests, follow-ups, flat limits, and omission counts. Path and blast-radius omissions may be lower bounds after bounded scans reach their caps.
