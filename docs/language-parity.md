...
- Native-only installs do not require `@lzehrung/codegraph-js-fallback` for normal JavaScript, TypeScript, or TSX import extraction. When JS query recovery is unavailable in `auto` mode, those languages degrade to regex import extraction and report the fallback in diagnostics instead of warning per file.
