...
- Thin snapshot hydrate now expands `export *` / `import *` after loading SQLite module bodies, then freezes the in-memory index. Disk cache rows are stored before that expansion, so skipping it made incremental updates throw when they tried to mutate frozen imports.
