...
Method invocations on a receiver are reported the same way as free-function calls. `callers` resolves a method call when the receiver is the type declaring the call (`this`, `$this`, `self`, `static`, `Self`, `cls`), one of its declared supertypes (`super`, `base`, `parent`), a named indexed type used as a static or class-qualified receiver, or a local whose constructor is proven (`const l = new Lib(); l.target()`). Inherited members resolve through declared `extends`, `implements`, `trait`, and `mixin` edges.
