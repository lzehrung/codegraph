...
describe("call compatibility fallback budget", () => {
  it("does not run verified callsite scanning after resolved refs produce a callsite", async () => {
    const root = await fsp.mkdtemp(path.join(os.tmpdir(), "dg-call-compat-fallback-budget-"));
    try {
      ...
      vi.mocked(findReferences).mockResolvedValue({
        status: "ok",
        references: [{ file: indexedMainFile, range: rangeFor(mainSource, "helper") }],
      });

      await attachCallCompatibilityHints(index, [changedSymbol], {
        maxRefs: 1000,
        projectRoot: root,
      });

      expect(changedSymbol.callCompatibility).toContainEqual(
