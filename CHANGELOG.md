...
- Certified `release` and `standalone-release` workflows retry GitHub artifact upload and download after transient artifact-service errors such as DNS `ENOTFOUND`. Native matrix jobs keep `fail-fast: false` and a 30-minute timeout. After a failed run, use **Re-run failed jobs** on the same Actions run so successful native artifacts are reused.
