...
import type { QueryIndexWorkerTask } from "./query-index-worker.js";
...
export function resolveQueryIndexWorkerPath(): string {
  ...
  const sibling = path.resolve(selfDir, "query-index-worker.js");
  if (fs.existsSync(sibling)) return sibling;
  const packageRoot = findPackageRoot(selfDir);
  const compiled = path.join(packageRoot, "dist", "agent", "query-index", "query-index-worker.js");
  if (fs.existsSync(compiled)) return compiled;
  const bundled = path.join(packageRoot, "dist", "bin", "query-index-worker.js");
