...
nativeDescribe("native parser ownership", () => {
  ...
  it(
    "keeps representative source-language navigation on the native runtime when the JS fallback package is unavailable",
    async () => {
      ...
      for (const testCase of cases) {
        ...
        if (testCase.symbolFile && testCase.symbolNames) {
          const seen = new Set(
            listSymbols(index, { file: normalizeFile(testCase.symbolFile) }).map(
              (symbol) => symbol.name,
            ),
          );
        ...
        expect(referencesResult.status).toBe(
          testCase.references.expectedStatus,
        );
      }
  ...
  it("builds detailed TypeScript symbol edges without loading the JS fallback package", async () => {
    ...
    const files = ["main.ts", "utils.ts", "helpers.ts"].map((file) =>
      path.join(root, file),
    );
