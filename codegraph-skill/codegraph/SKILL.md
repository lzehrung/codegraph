...
- `orient` reports only real build or update progress so warm cache hits stay quiet. Other commands that start by loading the project index report preparation on stderr when startup checks exceed a brief threshold, then report builds or updates automatically. Use `--progress` for redirected logs or `--no-progress` to suppress feedback, and JSON stdout remains unchanged.
