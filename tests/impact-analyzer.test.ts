...
describe("SQL object-level impact mapping", () => {
  async function writeSqlImpactFixture(root: string): Promise<{
    schemaFile: string;
    usersReader: string;
    ordersReader: string;
  }> {
    ...
    const usersReader = path.join(root, "db", "report.sql");
    const ordersReader = path.join(root, "db", "orders-report.sql");
    await fsp.writeFile(
      schemaFile,
      [
        "CREATE TABLE users (",
    ...
    await fsp.writeFile(usersReader, "SELECT id, email FROM users;\n", "utf8");
    await fsp.writeFile(ordersReader, "SELECT id FROM orders;\n", "utf8");
    return { schemaFile, usersReader, ordersReader };
  }

  it("maps a changed SQL object to its readers with a symbol-level reason", async () => {
    const root = await fsp.mkdtemp(path.join(os.tmpdir(), "cg-sql-impact-read-object-"));
    try {
      await writeSqlImpactFixture(root);
      const index = await buildProjectIndex(root, { cache: "memory" });
      const diffText = [
      ...
      expect(users?.range.start.line).toBe(1);
      expect(users?.range.end.line).toBeGreaterThanOrEqual(3);
      expect(users?.changedLines).toEqual(expect.arrayContaining([3]));
      ...
      expect(result.diagnostics?.changedFilesWithoutSymbols).toBe(0);
    } finally {
      await fsp.rm(root, { recursive: true, force: true, maxRetries: 5, retryDelay: 100 });
    }
  });

  it("does not impact a reader of a different object in the same changed file", async () => {
    const root = await fsp.mkdtemp(path.join(os.tmpdir(), "cg-sql-impact-unreferenced-object-"));
    try {
      await writeSqlImpactFixture(root);
      const index = await buildProjectIndex(root, { cache: "memory" });
      const diffText = [
      ...
      expect(result.changedSymbols.some((symbol) => symbol.name === "orders")).toBe(true);
      expect(result.changedSymbols.some((symbol) => symbol.name === "users")).toBe(false);
      expect(result.impacted.some((item) => item.file === "db/report.sql")).toBe(false);
      expect(result.impacted.some((item) => item.file === "db/orders-report.sql")).toBe(true);
    } finally {
      await fsp.rm(root, { recursive: true, force: true, maxRetries: 5, retryDelay: 100 });
    }
  });

  it("falls back to file-level impact for unparseable SQL with no mapped objects", async () => {
    const root = await fsp.mkdtemp(path.join(os.tmpdir(), "cg-sql-impact-unparseable-"));
    try {
      await fsp.mkdir(path.join(root, "db"), { recursive: true });
      await fsp.writeFile(
      ...
      await fsp.writeFile(path.join(root, "db", "report.sql"), "SELECT id FROM users;\n", "utf8");
      ...
      expect(result.diagnostics?.changedFilesWithoutSymbols).toBe(1);
      const reportItem = result.impacted.find((item) => item.file === "db/report.sql");
      expect(reportItem?.reasons).toEqual(["fileLevelChange"]);
    } finally {
      await fsp.rm(root, { recursive: true, force: true, maxRetries: 5, retryDelay: 100 });
    }
  });
