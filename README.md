...
- Safe refactors: get a handle with `codegraph list-symbols`, preview with `codegraph refactor rename --symbol <handle> --to <name> --json` or `codegraph refactor move --symbol <handle> --to-file <path> --json`, then add `--apply` when the edit set is acceptable.
