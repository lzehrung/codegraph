...
- Method-like declarations are indexed as function locals across supported source languages where the grammar exposes named methods or member functions. Body edits map to the method local; receiver-aware JS/TS method references and call compatibility are emitted only for verified receivers such as `new Service().run()` and `const service = new Service(); service.run()`.
