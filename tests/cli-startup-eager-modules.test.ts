...
function countDistModulesLoaded(args: string[]): { count: number; stdout: string; status: number | null } {
...
`,
  ...
  try {
    ...
    const match = /MODULE_COUNT=(\d+)/.exec(result.stderr ?? "");
    if (!match) {
      throw new Error(
        `Failed to count modules for ${args.join(" ")}. status=${result.status} stderr=${result.stderr}`,
      );
    }
    return { count: Number(match[1]), stdout: result.stdout ?? "", status: result.status };
  } finally {
...
describe("CLI startup eager module loading", () => {
  it("keeps command handlers behind dynamic import()", async () => {
