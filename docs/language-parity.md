...
- Receiver-aware method references cover JS/TS plus Java, C#, Rust, and Python `self.member()` calls within the declaring class. Python does not claim arbitrary receiver inference. Method-level call compatibility remains JS/TS-only and is emitted only for verified receivers such as `new Service().run()` and `const service = new Service(); service.run()`.
- Impact and review `diagnostics.memberResolutionCoverage` reports JS/TS, Java, C#, Rust, and Python as `receiverAwareLanguages`; every other source language among changed files is reported in `limitedLanguages`, meaning consumers reached only through a receiver expression (`obj.method()`) may be missing from the ranked impact list rather than merely lower-confidence.
