...
- Source discovery no longer stats every Git-listed file when looking for directory symlinks, and no longer walks default-ignored trees such as virtualenvs just to collect nested `.gitignore` files unless `includeGlobs` re-opens that tree. A Python repo with thousands of JSON or CSV files paid one `lstat` per data file during the symlink probe; those files are skipped, while Git mode 120000 still screens extension-bearing directory links.
