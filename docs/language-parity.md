...
- Native-only installs do not require `@lzehrung/codegraph-js-fallback` for normal JavaScript, TypeScript, or TSX import extraction, symbol indexing, chunking, or AST grep. When JS-family query recovery degrades in `auto` mode, those languages report the fallback in diagnostics instead of warning per file and stay on native-owned recovery paths.
