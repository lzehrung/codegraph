  ...
  it(
    ...
    async () => {
      ...
      try {
        ...
        const limitedFile = path.join(root, "helper.c");
        await fsp.writeFile(
          serviceFile,
        ...
        await fsp.writeFile(limitedFile, "int helper(const char *a, int b) {\n  return b;\n}\n", "utf8");
        ...
        const diffText = [
          ...
          "+++ b/helper.c",
          "@@ -1,3 +1,3 @@",
          "-int helper(const char *a) {",
          "+int helper(const char *a, int b) {",
          "   return b;",
          " }",
          "",
        ].join("\n");
        ...
        expect(stdout).toContain(
          "Note: limited receiver-call resolution for: c; consumers reached only through a receiver (e.g. obj.method()) may be missing from this report.",
        );
      } finally {
        await fsp.rm(root, { recursive: true, force: true });
      }
    ...
    async () => {
      ...
      try {
        ...
        expect(result.stdout).toContain(
          "limited receiver-call resolution: c (consumers reached only through a receiver, e.g. obj.method(), may be missing)",
        );
      } finally {
        await fsp.rm(root, { recursive: true, force: true });
      }
