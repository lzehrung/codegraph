...
Agent CLI commands use the incremental index path and default to disk cache. Pass shared index flags such as `--cache`, `--threads`, `--native`, `--workers`, `--include-glob`, or `--ignore-glob` when `orient`, `search`, `explain`, `packet get`, or `artifact build` should mirror a specific scan mode.
