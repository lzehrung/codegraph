...
- PHP dependency extraction, symbol lists, and compact and detailed graphs preserve `class`, `function`, and `const` import roles when targets or aliases share a spelling. Each import handle selects its own definition and references; an untyped PHP named import uses the class role.
