...
# List symbol handles, optionally expanding ranges to include attached docs
...
`refactor` commands require the companion `@lzehrung/codegraph-refactor` package to be installed alongside the CLI package.

`refactor rename` accepts definition handles from `list-symbols` or `--at <file>:<line>:<column>` locations resolved through go-to-definition. It returns deterministic text edits by default and rejects obvious target-name collisions in the declaring file. Add `--apply` to write the edits to disk. When a refactor creates new files, `--git` with `--apply` stages those new files. Text output reports unsupported or error results with status, reason, and warnings; applied text output also lists written, conflicted, skipped, and warning details.
...
`refactor move` supports TypeScript and JavaScript top-level declarations in v1. It accepts `--symbol` or `--at`, moves the trivia-aware declaration text, creates the target file when needed, carries imports used by the moved declaration into the target file, and rewrites named ES importers that Codegraph can resolve.

`refactor extract` supports inclusive contiguous TypeScript and JavaScript statement line ranges inside one named function declaration body. v1 does not extract from methods, arrows, or function expressions. It preserves simple TypeScript parameter annotations in generated helpers and rejects early `return`, unsupported control-flow, context-sensitive bindings, and selected declarations used after the range. It emits edits only; apply them with `--apply` after reviewing the JSON or diff.
