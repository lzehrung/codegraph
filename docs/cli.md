...
`refactor rename` accepts definition handles from `list-symbols` or `--at <file>:<line>:<column>` locations resolved through go-to-definition. It returns deterministic text edits by default and rejects obvious target-name collisions in the declaring file. Add `--apply` to write the edits to disk. When a refactor creates new files, `--git` with `--apply` stages those new files. Text output reports unsupported or error results with status, reason, and warnings.
