...
function resolveViewerOptions(options: ViewerServerOptions): ResolvedViewerOptions {
  ...
  if (options.graph !== undefined) {
    const lexicalGraphPath = assertFilePathWithinRoot(lexicalRoot, options.graph, "Graph");
    graphPath = assertFilePathWithinRoot(root, fs.realpathSync.native(lexicalGraphPath), "Graph");
    if (!fs.statSync(graphPath).isFile()) {
      throw new Error(`Graph must be a regular file: ${graphPath}`);
    }
    fs.accessSync(graphPath, fs.constants.R_OK);
  }
...
function viewerRequestHandler(
  ...
  graphFileDescriptor: number | undefined,
  ...
  return (request, response) => {
    ...
    if (pathname === "/graph.json") {
      if (!options.graphPath || graphFileDescriptor === undefined) {
        response.writeHead(404);
        response.end();
        return;
      }
      writeFileResponse(request, response, options.graphPath, "application/json; charset=utf-8", graphFileDescriptor);
      return;
    }
...
export function createViewerServer(options: ViewerServerOptions): http.Server {
  ...
  const graphFileDescriptor =
    resolvedOptions.graphPath === undefined ? undefined : fs.openSync(resolvedOptions.graphPath, "r");
  if (graphFileDescriptor !== undefined && !fs.fstatSync(graphFileDescriptor).isFile()) {
    fs.closeSync(graphFileDescriptor);
    throw new Error(`Graph must be a regular file: ${resolvedOptions.graphPath}`);
  }

  let allowedHostHeaders: AllowedHostHeaderRules | undefined;
  const host = options.host ?? DEFAULT_VIEWER_HOST;
  const server = http.createServer();
  const handler = viewerRequestHandler(resolvedOptions, graphFileDescriptor, () => {
...
export async function handleViewerCommand(context: ViewerCommandContext): Promise<void> {
  ...
  try {
    ...
    const hasGraph = resolveViewerOptions(options).graphPath !== undefined;
    if (preview) {
