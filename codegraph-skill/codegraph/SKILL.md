...
Use line and column coordinates when known; this is the primary navigation form. Use an exact project-relative `file::symbol` path when coordinates are unavailable; it cannot be combined with line or column input. `deps` and `rdeps` accept either form or a portable `symbol:` handle and resolve it to its defining file, while `callers` and `callees` expose symbol-level calls. They show depth 1 by default; use `--depth <n>` for another finite output depth or `--all` to include every depth. Their JSON contract changed from a bare `[{ depth, file }]` array to `{ items: [{ depth, file }], truncated }`. `truncated` is always a boolean: it is `true` when the selected finite depth omitted deeper entries and `false` for complete or empty results. Use `--all` when an agent needs the complete set. Duplicate local names return declaration candidates; run `codegraph symbols` to get a portable handle.
...
Use `--no-update-gitignore` to opt out during `init` or `sync --init`; ordinary `sync` never updates ignore policy. `uninit` preserves `.codegraph/server.json` and `.codegraph/server.log` even with `--force`, so stop a shared server explicitly; it leaves the root rule, while `init` and `sync` may warm or update `.codegraph-cache/index-v1/`.
