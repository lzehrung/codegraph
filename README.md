...
- Optional trivia-aware symbol ranges for selecting declarations with attached docs and decorators.
...
- Safe refactors: get a handle with `codegraph list-symbols`, preview with `codegraph refactor rename --symbol <handle> --to <name> --json`, `codegraph refactor move --symbol <handle> --to-file <path> --json`, or `codegraph refactor extract --file <path> --range <start:end> --to <name> --json`, then add `--apply` when the edit set is acceptable.
