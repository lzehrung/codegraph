...
Use the manually triggered `release` GitHub Actions workflow for a certified package release. Select `release_type=patch|minor|major`; standalone enrichment runs automatically after package publication and cannot block the registry write.
...
After the package release succeeds, the same workflow invokes `standalone-release` with the new `v<version>` tag. `standalone-release` also retains a manual trigger so failed standalone enrichment can be retried without republishing packages.
...
The standalone publish job attaches that post-smoke artifact to the existing GitHub Release. It does not rebuild or repackage standalone assets after smoke; a standalone failure makes the release run report failed enrichment, but the packages and GitHub Release already published by the preceding job remain available.
