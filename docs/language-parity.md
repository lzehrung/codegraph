...
- A few deeper semantic shapes remain intentionally limited and are covered by explicit regression tests instead of optimistic support claims: macro-heavy or otherwise non-local C typedef reference recovery beyond the current declaration use-site coverage.
