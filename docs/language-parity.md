...
- Type hierarchy fixtures prove TypeScript `extends`/`implements`, Java `extends`/`implements`, C# base-list inheritance and interface conformance, Rust `impl Trait for Type`, Swift class inheritance and protocol conformance, C++ base-class clauses, Python class bases, and Kotlin delegation specifiers.
- These forms emit resolved `extends` or `implements` edges only when both declarations are indexed. External bases, structural or duck typing, dynamic mixins, protocol extensions, overload/override dispatch, and name-only member matches are not inferred.
- Hierarchy member implementation lookup is limited to members owned by an interface or trait with proven implementers. Reduced graph-only recovery does not claim hierarchy parity.
