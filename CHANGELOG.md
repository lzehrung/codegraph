...
- PHP references find global-namespace symbols and case-variant spellings. A consumer without a `use` statement was never treated as a candidate file, and PHP class, function, and namespace names were compared case-sensitively although the language is not. A PHP `trait` is now a `class`-kind symbol, matching Rust: it previously collapsed to `variable` through the indexer's kind mapping, which made trait names compare case-sensitively and dropped legal case-variant references.
